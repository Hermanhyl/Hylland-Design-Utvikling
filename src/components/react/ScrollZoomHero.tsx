import { useEffect, useRef } from 'react';
import { motion, useScroll, useTransform, useReducedMotion } from 'framer-motion';
import Hero3D from './Hero3D';
import { MotionHeading, MOTION } from './motion-primitives';

/**
 * Scroll-zoom hero. Owns the scroll progress source of truth and drives:
 *  - the WebGL camera dolly / object morph (via the scrollYProgress MotionValue
 *    handed to Hero3D, read imperatively in its useFrame — no React re-renders)
 *  - the foreground headline scale + fade
 *
 * Uses native position:sticky + a tall runway (NO scroll-jacking), so it
 * coexists with the Nav scroll listener and Astro view transitions.
 * Mounted as client:only="react" (WebGL can't be server-rendered); the
 * server-rendered .hero-poster in index.astro provides the SEO/no-JS headline
 * and is hidden once this island sets [data-hydrated] on the shell.
 */
export default function ScrollZoomHero() {
	const heroRef = useRef<HTMLDivElement>(null);
	const reduced = useReducedMotion();

	const { scrollYProgress } = useScroll({
		target: heroRef,
		offset: ['start start', 'end start'],
	});

	// Foreground text: grows slightly and fades as you scroll into the next section.
	const textScale = useTransform(scrollYProgress, [0, 1], [1, 1.22]);
	const textOpacity = useTransform(scrollYProgress, [0, 0.55, 0.9], [1, 1, 0]);
	const textY = useTransform(scrollYProgress, [0, 1], [0, -60]);
	const stageOpacity = useTransform(scrollYProgress, [0, 0.85, 1], [1, 1, 0]);

	// Hide the static SEO poster once the island has mounted.
	useEffect(() => {
		const shell = document.querySelector<HTMLElement>('[data-hero]');
		if (shell) shell.setAttribute('data-hydrated', 'true');
		return () => {
			shell?.removeAttribute('data-hydrated');
		};
	}, []);

	const Foreground = (
		<div className="hero-content">
			<p className="hero-eyebrow">Frilans · Drammen · 2026</p>
			<h1 className="hero-title">
				<MotionHeading text="Frontend-utvikling og UX-design" />
				<span className="hero-title-accent italic">
					<MotionHeading text="fra Drammen." delay={0.35} />
				</span>
			</h1>
			<motion.p
				className="hero-lede"
				initial={reduced ? false : { opacity: 0, y: 16 }}
				animate={reduced ? undefined : { opacity: 1, y: 0 }}
				transition={{ duration: MOTION.duration, ease: MOTION.easeOut, delay: 0.6 }}
			>
				Jeg hjelper bedrifter med å bygge nettløsninger som er raske, tydelige og lette å bruke.
			</motion.p>
			<motion.div
				className="hero-cta"
				initial={reduced ? false : { opacity: 0, y: 16 }}
				animate={reduced ? undefined : { opacity: 1, y: 0 }}
				transition={{ duration: MOTION.duration, ease: MOTION.easeOut, delay: 0.75 }}
			>
				<a href="/tjenester" className="link-animated">Se tjenester →</a>
				<span className="hero-cta-sep">·</span>
				<a href="/kontakt" className="link-animated">Ta kontakt →</a>
			</motion.div>
		</div>
	);

	// Reduced motion: a single static viewport, no runway, no scroll coupling.
	if (reduced) {
		return (
			<div className="hero-runway hero-runway--static">
				<div className="hero-stage">
					<Hero3D className="hero-canvas" />
					<div className="hero-overlay">{Foreground}</div>
				</div>
			</div>
		);
	}

	return (
		<div ref={heroRef} className="hero-runway">
			<motion.div className="hero-stage" style={{ opacity: stageOpacity }}>
				<Hero3D className="hero-canvas" scrollProgress={scrollYProgress} />
				<motion.div
					className="hero-overlay"
					style={{ scale: textScale, opacity: textOpacity, y: textY }}
				>
					{Foreground}
				</motion.div>
			</motion.div>
		</div>
	);
}
